...
export default function MaterialReviewPage() {
 ...
 <p className="text-gray-500 dark:text-slate-400">Material not found</p>
 ...
 <Button variant="outline" onClick={handleDeleteMaterial} className="gap-1 text-red-600 dark:text-red-400 hover:bg-red-50 dark:bg-red-900/20 hover:text-red-700 border-red-200">
 ...
 <div className="space-y-2">
 <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Summary</label>
 <Textarea value={summary} onChange={(e) => setSummary(e.target.value)} rows={4} />
 </div>
 <div className="grid gap-4 sm:grid-cols-2">
 <div className="space-y-2">
 <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Keywords (comma-separated)</label>
 <Input value={keywords} onChange={(e) => setKeywords(e.target.value)} placeholder="keyword1, keyword2" />
 </div>
 <div className="space-y-2">
 <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Topics (comma-separated)</label>
 <Input value={topics} onChange={(e) => setTopics(e.target.value)} placeholder="topic1, topic2" />
 </div>
 </div>
 <div className="grid gap-4 sm:grid-cols-2">
 <div className="space-y-2">
 <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Tags (comma-separated)</label>
 ...
 <CardContent>
 {quizzes.length === 0 ? (
 <p className="text-sm text-gray-400 dark:text-slate-500">No quizzes generated for this material</p>
 ) : (
 <div className="space-y-4">
 {quizzes.map((q) => (
 <div key={q.id} className="rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
 {/* Quiz Header */}
 <button
 onClick={() =>
 setExpandedQuiz(expandedQuiz === q.id ? null : q.id)
 }
 className="flex items-center justify-between w-full p-4 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors text-left cursor-pointer"
 >
 <div>
 ...
 {expandedQuiz === q.id && (
 <div className="border-t border-gray-100 dark:border-slate-700 p-4 bg-gray-50/50">
 <QuizQuestionEditor
 quiz={q}
